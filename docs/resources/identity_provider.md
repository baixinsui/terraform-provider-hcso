...
Manages the identity providers within Huawei Cloud Stack Online IAM service.
...
* `sso_type` - (Optional, String, ForceNew) Specifies the single sign-on type of the identity provider.
  ...
  + **virtual_user_sso**: After a federated user logs in to Huawei Cloud Stack Online, the system automatically creates a virtual user
    and assigns permissions to the user based on identity conversion rules.
  + **iam_user_sso**: After a federated user logs in to Huawei Cloud Stack Online, the system automatically maps the external identity

...
func TestAccIdentityAgenciesDataSource_basic(t *testing.T) {
	dataSourceName := "data.hcso_identity_agencies.all"
...
func TestAccIdentityAgenciesDataSource_byName(t *testing.T) {
	dataSourceName := "data.hcso_identity_agencies.query_by_name"
...
func testAccIdentityAgenciesDataSource_byName(rName string) string {
	return fmt.Sprintf(`
resource "hcso_identity_agency" "test" {
  name                  = "%s"
...
data "hcso_identity_agencies" "query_by_name" {
  depends_on = [
    hcso_identity_agency.test
  ]
  name = hcso_identity_agency.test.name
}
`, rName, acceptance.HCSO_DOMAIN_NAME, acceptance.HCSO_REGION_NAME)
}

func TestAccIdentityAgenciesDataSource_byTrustDomainId(t *testing.T) {
	dataSourceName := "data.hcso_identity_agencies.query_by_trust_domain_id"
	dc := acceptance.InitDataSourceCheck(dataSourceName)
...
data "hcso_identity_agencies" "query_by_trust_domain_id" {
  depends_on = [
    hcso_identity_agency.test
  ]
  trust_domain_id = "%s"
}
`, rName, acceptance.HCSO_DOMAIN_NAME, acceptance.HCSO_REGION_NAME, acceptance.HCSO_DOMAIN_ID)
}
